...
const SAVE_FORM = document.getElementById('saveForm'),
 ...
 NOMBRE_PIEZA = document.getElementById('AgregarPieza');
 DESCRIPCION = document.getElementById('AgregarDescripcion');
 PRECIO = document.getElementById('AgregarPrecio');
...
SAVE_FORM.addEventListener('submit', async (event) => {
    ...
    (ID_PIEZA.value) ? action = 'updateRow' : action = 'createRow';
    // Constante tipo objeto con los datos del formulario.
    const FORM = new FormData(SAVE_FORM);
    // Petición para guardar los datos del formulario.
    const DATA = await fetchData(PIEZA_API, action, FORM);
        ...
        DATA.dataset.forEach(row => {
            ...
            TABLE_BODY.innerHTML += `
                <tr>
                    ...
                    <td>
                        <button id="btn1" type="button" class="btn" onclick="openUpdate(${row.id_pieza})">
                            <i class="bi bi-pencil-fill"></i>
                        </button>
                        <button id="btn2" type="button" class="btn" onclick="openDelete(${row.id_pieza})">
                            <i class="bi bi-trash-fill"></i>
                        </button>
                        <button id="btn1" type="button" class="btn" onclick="openChart(${row.id_pieza})">
...
const openUpdate = async (id) => {
    ...
    const DATA = await fetchData(PIEZA_API, 'readOne', FORM);
    // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con la excepción.
    if (DATA.status) {
        // Se muestra la caja de diálogo con su título.
        SAVE_MODAL.show();
        ...
        DESCRIPCION.value = ROW.descripcion_pieza;
...
const openDelete = async (id) => {
    ...
    if (RESPONSE) {
        ...
        const DATA = await fetchData(PIEZA_API, 'deleteRow', FORM);
        // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con la excepción.
        if (DATA.status) {
            // Se muestra un mensaje de éxito.
...
const openChart = async (id) => {
    ...
    if (DATA.status) {
        ...
        lineChart('chart', modelos, coches, 'Cantidad de vehículos', 'Unidades utilizadas por modelo');
    } else {
        sweetAlert(4, DATA.error, true);
    }
}
